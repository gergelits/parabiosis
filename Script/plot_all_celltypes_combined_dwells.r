...
read_dwell_cd69p <- function( d_res, m_tisNA )
{
  ...
  for ( i in 1 : nrow( d_res ) ) {
    ...
    fig_fname_start <- get_fig_fname_start( m = m_tisNA )   # TODO rename all fig_filename_start -> fig_fname_start
                            ...
                            n_sim = 10000, dwell_max = 120 )
{
  ...
  if ( unique( d_dwells$type ) == "combined" ) { 
    pdf_name <- sprintf( "%s/%s_%isims_Figure_6C_comb_dwell_medians_v08.pdf", 
                         figs_comb_dir, fig_fname_start, n_sim )
  }
  if ( unique( d_dwells$type ) == "cd69p" ) { 
    pdf_name <- sprintf( "%s/%s_Figure_6D_cd69p_dwell_medians_v08.pdf", 
