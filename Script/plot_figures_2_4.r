...
plot_figures_2_4 <- function( m_tisNA = NULL, qij_I = 1L )
{
  ...
  # dTMP.2 %>% filter( is.na( hdi.80.lower ) )
  ...
  if ( FIG_DIAG_18_PARAMS <- TRUE ) {
    ...
    # if ( qij_I == 1 ) {
    #   pdf_name <- 
    #     sprintf( "%s/%s_diag_18_params.pdf", figs_dir, fig_filename_start ) 
    #   } else {
    #     pdf_name <- 
    #       sprintf( "%s/%s_diag_18_params_PO.pdf", figs_dir, fig_filename_start ) 
    #   }
