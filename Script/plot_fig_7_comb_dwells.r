                                    ...
                                    ylim_max = 50 )
{
  ...
  stan_pars_v_0115 <- "v08"
  m_tisNA_0115 <- model( 
    model = m_tisNA, 
    model_name = model_name,
    stan_pars_v = stan_pars_v_0115, 
    model_ver = sprintf( "%s_t%s", stan_pars_v_0115, N_TISSUES = 1 ) )

  for ( celltype in celltypes ) {
    
    m_tisNA <- m_tisNA_0115
  ...
  if ( export ) {
    ...
    pdf_name <- sprintf( 
      "%s/%s_%isims_Figure_7_comb_dwell_examples_%s_vs8.pdf", 
