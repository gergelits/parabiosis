                                ...
                                Q_from_HDI = TRUE )
{
  ...
  source( sprintf( "%s/%s", m$ps$CODE_PATH, "compare_qij_and_mode_TESTING.r"), 
          local = TRUE ) 
...
{
  model_id<-sprintf("%s_m0115_v08_t1_i1001",celltype)
                           ...
                           x_max_cut = 1000 )
{
  ...
  ggsave( filename = sprintf( "./Results/Combined/lifespan_%s_%s_%ssim_max%s_v9.pdf",
                              celltype, state, 
...
if ( RUN_TEST <- FALSE ) {
  ...
  ggsave( filename = sprintf( 
    "./Results/Combined/lifespan_%s_%s_%isim_max%s_v11.pdf",
